...
# --- SOZLAMALAR ---
API_TOKEN = '8611357164:AAHkJ0YywP7zvKW4nGY84dRsMSMva_pTOfM'  # <-- O'z tokeningizni yozing
ADMIN_ID = 7250754904
CHANNELS = ["@pulishla_z_community"]  # <-- Kanal username
REFERRAL_SUM = 3000
MIN_WITHDRAW = 50000
...
async def casino(message: types.Message):
    ...
    # G'alaba ehtimoli: balansi kamligi uchun ko'proq yutadi (saqlash)
    chance = 45  # 45% yutish ehtimoli
...
async def admin_stats(message: types.Message):
    ...
    row = sql.fetchone()
    count, total_bal, total_refs = row
    await message.answer(
        f"📊 <b>Bot statistikasi</b>\n\n"
        f"👥 Foydalanuvchilar: <b>{count}</b>\n"
        f"💰 Jami balans: <b>{total_bal or 0:,} so'm</b>\n"
        f"🔗 Jami referallar: <b>{total_refs or 0}</b>"
    )
